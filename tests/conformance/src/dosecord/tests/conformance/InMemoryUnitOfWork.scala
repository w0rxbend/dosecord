...
/** In-memory port fakes for the conformance harness (suite B2's mediator-resolution scenarios and the adapters'
  * end-to-end suites). Mirrors core's test `MediatorFakes` — test sources are not shared across modules — covering
  * every port the real handler compositions touch.
  */
...
/** In-memory account creates; `displayName` stays None — the neutral default the Postgres write guarantees by never
  * selecting the vendor nickname (K8).
  */
...
final class InMemoryAccounts(identities: InMemoryIdentities) extends AccountRepository:
  ...
  override def timezoneOf(accountId: UUID): Option[String] =
    this.synchronized(rows.find(_.accountId == accountId).map(_.currentTimezone))
...
final class InMemoryOccurrences extends OccurrenceRepository:
  ...
  override def cancel(ids: List[UUID], reason: dosecord.core.domain.CancelReason, now: Instant): Int = this.synchronized:
    var cancelled = 0
    rows.mapInPlace { r =>
      if ids.contains(r.id) && r.status.isOpen then
        ...
        r.copy(
          ...
          version = r.version + 1,
          state = r.state.copy(status = dosecord.core.domain.OccurrenceStatus.Cancelled, nextActionAt = None, epoch = r.state.epoch + 1)
        )
      else r
    }
    cancelled
    ...
    this.synchronized:
      rows.exists(r =>
        r.scheduleId.contains(scheduleId) && r.localDate == localDate &&
          ((r.status != dosecord.core.domain.OccurrenceStatus.Pending && r.status != dosecord.core.domain.OccurrenceStatus.Cancelled) ||
